        ...
        if not (False in list(np.greater_equal(avail, n_need[i]))):
            ...
            if processes.index(_max) > processes.index(i):  # if true, i is older
                ...
                if work[ind] == 'w':
                    ...
                    # logger.info('offload reentry: ', i, offload)
                else:
...
def save_and_send(send_path):
    ...
    file_ = open(f'{_id_}_16_{mec_no}datap.py', 'w')
    for i in list_result:
        file_.write(i)
    file_.close()
    cmd = f'mv {_id_}_16_{mec_no}datap.py {send_path}'
    os.system(cmd)
