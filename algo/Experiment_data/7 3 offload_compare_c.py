...
def histogram(local, offload, ax, no):
    ...
    ax.set_xticks(ind)
    ax.set_xticklabels(algo_dict.values(), fontdict={'size': 14})
    a = []
    for i in local:
        if i in a:
            ...
            ax.text(j, offload[j], '{}%'.format(percent(offload[j], total)), rotation=0,
                    ha="center", va="center", bbox=dict(boxstyle="round", ec=(1., 0.5, 0.5), fc=(1., 0.8, 0.8), ))
            a.append(i)
    ax.legend((p1[0], p2[0]), ('OP', 'LEP'), prop={"size":14})    # OT offloaded tasks, LET Locally Executed Tasks
    ...
    axx.set_ylabel(f'{no} MECs', rotation=0, fontsize=15, labelpad=30)
